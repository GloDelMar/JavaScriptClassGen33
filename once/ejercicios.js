...
//Ejercicio 1 Obtener una lista de los géneros sin repetir
const listaDeGeneros = (dataArray) =>
  dataArray.reduce(
    (accum, current) =>
      accum.includes(current.rating) ? accum : [...accum, current.rating],
    []
  );

let PeliPorGenero = listaDeGeneros(movies)

  //Ejercicio 2  Obtener una lista de los nombres de los directores sin repetir

  const listaDeDirectores = (dataArray) => dataArray.reduce(
    (accum, current) => accum.includes(current.director) ? accum: 
    [...accum, current.director],
    []
  )

 let PeliPorDirector = listaDeDirectores(movies)
 //Ejercicio 3  Obtener una lista con únicamente los nombres de los actores protagonistas de todas las películas, sin repetir
 
 
 const PreviaProtagonistas = (dataArray) => {
    let listaPrevia = dataArray.reduce(
        (accum, current) => accum.includes(current.protagonists) ? accum : [...accum, current.protagonists],
        []
    );
    return listaPrevia;
};

const laListadeProtagonistas = (listaPrevia) => {
    let result = listaPrevia.reduce(
        (accum, current) => accum.includes(current.arr.name) ? accum : [...accum, current.arr.name],
        []
    );
    return result;
};
...
//requiere corregir... ----->problemas aquí
  ...
  const listaDeRatings = (dataArray) => dataArray.reduce(
    (accum, current) => accum.includes(current.rating) ? accum: [...accum, current.rating],
    []
  )

 let losRating = listaDeRatings(movies)
  //ejercicio 5 Obtener una lista de las películas que tienen una duración entre 2 rangos, por ejemplo las películas que duran entre 100 y 120 mins ( los valores de las duraciones deben ser dinámicos)
  const ListapeliculasPorDuracion = (dataArray, duracionMinima, duracionMaxima) => {
    return dataArray.filter(pelicula => pelicula.duration >= duracionMinima && 
        pelicula.duration <= duracionMaxima)
}

let peliduracion = ListapeliculasPorDuracion(movies, "100 minutos", "200 minutos")

  //Ejercicio 6 Obtener una lista de películas con base en su clasificación
  const organizadoPorRating = (dataArray) => {
    let result = dataArray.reduce((accum, current)=>{
        return accum[current.rating] ? {...accum,[current.rating]: [...accum[current.rating], current],} : { ... accum, [current.rating] : [current]}
    }, {})
    return result
  }
let PeliPorRating = organizadoPorRating(movies)
  
  //Ejercicio 7 Obtener una lista de películas películas estrenadas en un rango de años (por ejemplo, entre 2000 - 2010, los valores de los años deben ser dinámicos)
  const ListaDePeliculasPorAños = (dataArray, añoMinimo, añoMaximo) => {
    return dataArray.filter(pelicula => pelicula.releaseYear >= añoMinimo && 
        pelicula.releaseYear <= añoMaximo)
  }
  let peliPorAño = ListaDePeliculasPorAños(movies, 2000, 2010)
  // Ejercicio 8 Obtener una lista de películas con base en el país al que pertenecen
  const organizadoPorPais= (dataArray) => {
    let result = dataArray.reduce((accum, current)=>{
        return accum[current.country] ? {...accum,[current.country]: [...accum[current.country], current],} : { ... accum, [current.country] : [current]}
    }, {})
    return result
  }

let pelisPorPais = organizadoPorPais(movies)
  //Ejercicio 9. Obtener una lista de las películas que no obtuvieron premios Óscar

  const ListapeliculasSinOscar= (dataArray) => {
    return dataArray.filter(pelicula => pelicula.oscarAwards === 0)
}
let sinOscares = ListapeliculasSinOscar(movies)

  //Ejercicio 10. Obtener la cantidad de películas de cada clasficación. Esta información debe estar organizada de la siguiente forma:
     /*  {
           [nombre_de_la_clasificacion]:[cantidad]
       }
